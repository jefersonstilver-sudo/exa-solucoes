-- Corrigir função reactivate_base_video_when_no_schedules para fazer JOIN correto
...
BEGIN
  ...
  -- Se não há mais regras ativas, reativar vídeo base
  IF v_remaining_schedules = 0 THEN
    PERFORM sync_base_video_to_api(v_pedido_id);
  END IF;
