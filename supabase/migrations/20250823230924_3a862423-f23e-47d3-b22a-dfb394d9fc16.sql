-- Criar função para classificar corretamente pedidos aguardando vídeo
...
BEGIN
  ...
  SELECT 
    ...
    CASE 
      WHEN p.status = 'pago' AND (SELECT COUNT(*) FROM pedido_videos pv WHERE pv.pedido_id = p.id) = 0 
