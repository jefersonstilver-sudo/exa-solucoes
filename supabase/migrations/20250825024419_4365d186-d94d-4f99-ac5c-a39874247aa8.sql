-- Criar função RPC para buscar vídeos aprovados por período
...
BEGIN
  ...
  -- Se não há filtro de data, usar últimos 30 dias por padrão
  IF p_start_date IS NULL AND p_end_date IS NULL THEN
    p_start_date := CURRENT_DATE - INTERVAL '30 days';
    p_end_date := CURRENT_DATE;
  END IF;

  -- Se só start_date foi fornecido, usar até hoje
  IF p_start_date IS NOT NULL AND p_end_date IS NULL THEN
    p_end_date := CURRENT_DATE;
  END IF;

  -- Se só end_date foi fornecido, usar últimos 30 dias
