-- Forçar recriação completa da função approve_video para limpar qualquer cache
DROP FUNCTION IF EXISTS public.approve_video(uuid, uuid) CASCADE;

-- Recriar função approve_video SEM qualquer referência a updated_at na tabela pedidos
...
BEGIN
  ...
  BEGIN
    -- Update the video approval status (pedido_videos TEM updated_at)
    ...
    -- Se este vídeo foi definido como base, desmarcar outros (pedido_videos TEM updated_at)
  ...
  IF v_is_first_approval THEN
    ...
    -- Atualizar datas do contrato (pedidos NÃO TEM updated_at)
    UPDATE pedidos 
    SET 
  ...
  -- Atualizar status do pedido (pedidos NÃO TEM updated_at)
  UPDATE pedidos 
...
VALUES ('MIGRATION', 'Forçada recriação completa da função approve_video (DROP + CREATE) para limpar cache e corrigir definitivamente erro de updated_at');