...
function generateContractHtml(contrato: any): string {
  ...
  return `
    ...
    <body>
      ...
      <h2>CLÁUSULA 1ª - DO OBJETO</h2>
      <p>1.1. O presente contrato tem por objeto a veiculação de anúncios publicitários em vídeo com duração de até 15 (quinze) segundos, fornecidos pela CONTRATANTE, nos painéis digitais da EXA MÍDIA, localizados em prédios residenciais da cidade de Foz do Iguaçu - PR, pelo prazo contratado de ${contrato.plano_meses || 1} (${contrato.plano_meses === 1 ? 'um' : contrato.plano_meses}) ${contrato.plano_meses === 1 ? 'mês' : 'meses'} de exibição.</p>
      
      <h2>CLÁUSULA 2ª - DOS LOCAIS CONTRATADOS</h2>
      ...
      <p>3.1. Valor mensal: <strong>${formatCurrency(contrato.valor_mensal)}</strong></p>
      <p>3.2. Valor total do contrato: <strong>${formatCurrency(contrato.valor_total)}</strong></p>
      <p>3.3. Forma de pagamento: ${(contrato.metodo_pagamento || 'a definir').replace(/_/g, ' ').toUpperCase()}</p>
      ${contrato.dia_vencimento ? `<p>3.4. Vencimento das parcelas: dia ${contrato.dia_vencimento} de cada mês.</p>` : ''}
      <p>3.5. Após 10 (dez) dias de atraso no pagamento, a exibição será automaticamente suspensa até a regularização.</p>
      <p>3.6. Multa por atraso: 2% (dois por cento) + 1% (um por cento) de juros ao mês.</p>
      
      <h2>CLÁUSULA 4ª - DO CONTEÚDO PUBLICITÁRIO</h2>
      <p>4.1. A CONTRATANTE compromete-se a enviar vídeos conforme especificações técnicas da CONTRATADA (resolução 1920x1080, formato MP4, máximo 15 segundos).</p>
      <p>4.2. Os vídeos podem ser substituídos a qualquer momento, mediante solicitação prévia.</p>
      <p>4.3. A CONTRATADA poderá recusar conteúdos que não atendam aos padrões técnicos ou que contenham material ofensivo, ilegal ou que infrinja direitos de terceiros.</p>
      
      <h2>CLÁUSULA 5ª - DO CANCELAMENTO</h2>
      <p>5.1. O cancelamento antecipado por parte da CONTRATANTE gerará multa de 30% (trinta por cento) sobre o saldo devedor restante.</p>
      
      <h2>CLÁUSULA 6ª - DO FORO</h2>
      <p>6.1. Para dirimir eventuais dúvidas ou litígios oriundos deste contrato, as partes elegem o foro da Comarca de Foz do Iguaçu - PR, com renúncia expressa a qualquer outro.</p>
      
      ${contrato.clausulas_especiais ? `<h2>CLÁUSULA 7ª - CONDIÇÕES ESPECIAIS</h2><p>${contrato.clausulas_especiais}</p>` : ''}
