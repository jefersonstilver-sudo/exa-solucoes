...
export const usePanelOperations = () => {
  ...
  const removePanel = useCallback(async (panelId: string, panelCode: string, buildingName?: string) => {
    if (operationRef.current.inProgress) {
      console.log('⏳ [PANEL OPERATIONS] Operação já em andamento');
    ...
    try {
      console.log('🔍 [PANEL OPERATIONS] Verificando campanhas ativas para painel:', panelCode);
      ...
      console.log('🔄 [PANEL OPERATIONS] Removendo atribuição do painel...');
      ...
      try {
        ...
        console.warn('⚠️ [PANEL OPERATIONS] Falha ao registrar log (não crítico):', logError);
      }
      ...
      console.error('💥 [PANEL OPERATIONS] Erro na remoção:', error);
  ...
  const syncPanel = useCallback(async (panelId: string) => {
    try {
      // Implementação futura do sync
      console.log('🔄 [PANEL OPERATIONS] Sync do painel:', panelId);
      toast.info('Funcionalidade de sincronização em desenvolvimento');
    } catch (error) {
      console.error('❌ [PANEL OPERATIONS] Erro no sync:', error);
      toast.error('Erro na sincronização do painel');
    }
  }, []);

  const viewPanelDetails = useCallback((panelId: string) => {
    // Implementação futura dos detalhes
    console.log('👁️ [PANEL OPERATIONS] Visualizar detalhes do painel:', panelId);
