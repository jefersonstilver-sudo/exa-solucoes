...
export const useVideoManagement = ({ orderId, userId, orderStatus, tipoProduto }: UseVideoManagementProps) => {
  ...
  const handleRemove = async (slotId: string) => {
    try {
      ...
      console.log(`🗑️ [useVideoManagement] Chamando deleteVideoWithExternalAPI: slotId=${slotId}, video_id=${slot?.video_id}, orderId=${orderId}`);
      await deleteVideoWithExternalAPI(slotId, slot?.video_id, orderId);
