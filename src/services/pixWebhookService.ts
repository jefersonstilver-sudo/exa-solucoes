  ...
  try {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        console.error("❌ Erro HTTP do N8N webhook TESTE:", {
          status: response.status,
          statusText: response.statusText,
          body: errorText,
          url: PIX_WEBHOOK_URL
        ...
        console.error("❌ Erro ao ler resposta de erro do N8N TESTE:", e);
      }
      throw new Error(`N8N Webhook TESTE falhou: ${response.status} - ${response.statusText}${errorText ? ` - ${errorText}` : ''}`);
    }
    
    let result;
    const responseText = await response.text();
    console.log("📄 RESPOSTA BRUTA DO N8N TESTE:", responseText);
    
    if (!responseText || responseText.trim() === '') {
      console.error("❌ N8N TESTE retornou resposta vazia");
      throw new Error("N8N webhook TESTE retornou resposta vazia");
    }
    
    try {
      result = JSON.parse(responseText);
      console.log("✅ JSON DO N8N TESTE PARSEADO:", result);
    } catch (parseError) {
      console.error("❌ Erro ao fazer parse do JSON do N8N TESTE:", parseError);
      console.error("❌ Resposta que causou erro:", responseText);
      
      if (responseText.includes('<html') || responseText.includes('<!DOCTYPE')) {
        throw new Error("N8N TESTE retornou uma página HTML em vez de JSON. Verifique se o webhook está ativo.");
      }
      
      throw new Error(`Resposta do N8N TESTE não é um JSON válido: ${parseError.message}`);
    }
    ...
    console.log("🔍 MAPEAMENTO CORRIGIDO - VERIFICAÇÃO DE DADOS PIX DO N8N TESTE:", {
      hasPixData,
    ...
    if (hasPixData) {
      ...
      console.log("✅ MAPEAMENTO CORRIGIDO - DADOS PIX MAPEADOS CORRETAMENTE (TESTE):", {
      ...
      console.error("❌ N8N TESTE não retornou dados PIX:", result);
      
      if (result.success === false && result.error) {
        throw new Error(`Erro do N8N TESTE: ${result.error}`);
      }
      
      throw new Error("N8N TESTE não retornou dados PIX válidos. Campos esperados: pix_base64, pix_url");
    }
    ...
    if (error.name === 'AbortError') {
      userMessage = "Timeout ao conectar com N8N TESTE. Tente novamente.";
    } else if (error.message.includes('fetch')) {
      userMessage = "Erro de conexão com N8N TESTE. Verifique sua internet.";
    } else if (error.message.includes('JSON')) {
      userMessage = "Erro na resposta do N8N TESTE. Tente novamente em alguns minutos.";
