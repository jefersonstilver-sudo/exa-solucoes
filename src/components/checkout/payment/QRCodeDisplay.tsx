...
export const QRCodeDisplay = ({
  ...
  return <motion.div className="flex flex-col items-center" initial={{
    ...
    duration: 0.3
      ...
      <div className={`border-2 rounded-lg p-4 shadow-sm relative ${imageLoaded ? 'border-green-200 bg-white' : 'border-gray-200 bg-gray-50'}`}>
        ...
        <img src={imageSource} alt="QR Code PIX" className={`w-56 h-56 rounded-lg transition-opacity duration-300 ${imageLoaded ? 'opacity-100' : 'opacity-0'}`} onError={handleImageError} onLoad={handleImageLoad} style={{
        display: imageError ? 'none' : 'block'
      }} />
      </div>
      
      {imageLoaded && <motion.div className="mt-3 flex items-center space-x-2" initial={{
      opacity: 0,
      y: 10
    }} animate={{
      opacity: 1,
