...
const AISection: React.FC = () => {
  ...
  return (
    <section className="py-20 bg-gradient-to-r from-gray-50 to-[#3C1361]/5">
      <div className="max-w-6xl mx-auto px-4">
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-bold mb-6">
            🤖 IA + MARKETING: <span className="text-[#3C1361]">A NOVA ERA É AGORA</span>
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
          {aiApplications.map((application, index) => (
            <div key={index} className="flex items-center bg-white rounded-lg p-4 border border-gray-200 shadow-sm">
              <Lightbulb className="h-6 w-6 text-[#3C1361] mr-4 flex-shrink-0" />
              <span className="text-gray-900">{application}</span>
            </div>
          ))}
        </div>

        <div className="bg-[#3C1361]/10 border border-[#3C1361]/30 rounded-2xl p-8 text-center">
          <Gift className="h-12 w-12 text-[#3C1361] mx-auto mb-4" />
          <h3 className="text-2xl font-bold text-[#3C1361] mb-4">Manual Gratuito Exclusivo</h3>
