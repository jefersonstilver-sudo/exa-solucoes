...
export const VideoTitleInput: React.FC<VideoTitleInputProps> = ({
  ...
  return <div className="space-y-2">
      ...
      <div className="space-y-1">
        <Input id="video-title" type="text" value={title} onChange={e => onTitleChange(e.target.value)} placeholder={placeholder} maxLength={maxLength} className={`w-full ${error ? 'border-red-500' : isValid ? 'border-green-500' : ''}`} />
      ...
      {isValid && <p className="text-green-600 text-xs">✓ Título válido</p>}
    </div>;
};