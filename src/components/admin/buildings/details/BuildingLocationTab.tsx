...
const BuildingLocationTab: React.FC<BuildingLocationTabProps> = ({
  ...
  const [savedPositionLocal, setSavedPositionLocal] = useState<{
    lat: number;
    lng: number;
  } | null>(null);
  ...
  const getCurrentCoords = () => {
    ...
    if (savedPositionLocal) {
      return {
        lat: savedPositionLocal.lat,
        lng: savedPositionLocal.lng,
        isManual: true
      };
    }
  ...
  const handleSave = async () => {
    const positionToSave = draggedPosition || {
      lat: currentCoords.lat,
      lng: currentCoords.lng
    };
    setIsSaving(true);
  ...
  return <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              ...
              <Button onClick={handleSave} disabled={isSaving || !hasUnsavedChanges} className="bg-green-600 hover:bg-green-700 disabled:opacity-50">
              ...
              <Button variant={isEditing ? "default" : "outline"} onClick={() => setIsEditing(!isEditing)} className={isEditing ? "bg-blue-600 hover:bg-blue-700" : ""}>
                <MapPin className="h-4 w-4 mr-2" />
                {isEditing ? 'Editando' : 'Editar Posição'}
              </Button>
              
              {/* Reset button when editing or has changes */}
              {(isEditing || hasUnsavedChanges) && <Button variant="outline" onClick={handleReset}>
        ...
        <CardContent>
          {/* Edit Mode Indicator */}
          {isEditing && <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-md">
          ...
          {hasUnsavedChanges && <div className="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-md">
          ...
          <div ref={mapRef} className="w-full h-96 rounded-md border bg-muted" />
