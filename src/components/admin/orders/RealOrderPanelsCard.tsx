...
import { MonitorPlay, MapPin, Building, Calendar } from 'lucide-react';
    ...
    <Card className="bg-white border-gray-200">
      <CardHeader className="border-b border-gray-200">
        <CardTitle className="text-gray-900 flex items-center">
          <MonitorPlay className="h-5 w-5 mr-2 text-indexa-purple" />
          Painéis Contratados
        </CardTitle>
        <CardDescription className="text-gray-600">
          {panels.length} {panels.length === 1 ? 'painel contratado' : 'painéis contratados'} para {order.plano_meses} {order.plano_meses === 1 ? 'mês' : 'meses'}
        </CardDescription>
      </CardHeader>
      <CardContent className="p-6">
        {panels.length === 0 ? (
          <div className="text-center py-8">
            <MonitorPlay className="h-12 w-12 mx-auto text-gray-400 mb-4" />
            <p className="text-gray-600">Nenhum painel encontrado para este pedido</p>
          </div>
        ) : (
                  ...
                  <div>
                    ...
                    <div className="flex items-center space-x-4 mt-1">
                      <div className="flex items-center space-x-1">
                        ...
                        <p className="text-sm text-gray-600">{panel.building_name}</p>
                      </div>
                      <div className="flex items-center space-x-1">
                        <MapPin className="h-3 w-3 text-gray-500" />
                        <p className="text-sm text-gray-600">{panel.building_neighborhood}</p>
                      </div>
                    </div>
                    <p className="text-xs text-gray-500 mt-1">{panel.building_address}</p>
                  </div>
                </div>
