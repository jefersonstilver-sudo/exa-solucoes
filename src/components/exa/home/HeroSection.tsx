...
const VideoRetryOverlay = ({ onRetry }: { onRetry: () => void }) => (
  <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-black/70 backdrop-blur-sm gap-3">
    <p className="text-white text-sm font-medium">Vídeo indisponível</p>
    <button
      onClick={onRetry}
      className="flex items-center gap-2 px-4 py-2 rounded-full bg-white/15 backdrop-blur-md border border-white/25 hover:bg-white/25 transition-all text-white text-sm"
    >
...
// Mobile Hero Layout
// ══════════════════════════════════════════════════════════
const HeroMobileLayout = ({ overrideUrl }: { overrideUrl?: string }) => {
  ...
  return (
    <section className="bg-gradient-to-b from-[#9C1E1E] via-[#180A0A]/80 to-exa-black pt-0 pb-2">
      <div ref={containerRef} className="relative w-screen left-1/2 -translate-x-1/2">
        ...
        >
          ...
          />
          {hasError && <VideoRetryOverlay onRetry={manualRetry} />}
          {isRecovering && !hasError && <RecoveringIndicator />}
        </div>
      ...
      <div className="px-5 py-3 text-center space-y-2">
        ...
        <p className="font-poppins text-xs text-gray-200/90 leading-relaxed max-w-sm mx-auto">
          A EXA conecta marcas aos instantes reais da vida urbana — atenção genuína e resultados
          duradouros.
        </p>
      </div>
    </section>
...
const HeroSection = () => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => setShowSoundTooltip(false), 5000);
  ...
  // Desktop layout
  return (
    <section className="bg-gradient-to-b from-[#9C1E1E] via-[#180A0A]/80 to-exa-black min-h-screen flex items-center relative overflow-hidden">
      {/* Top gradient cover */}
      <div className="absolute top-0 left-0 right-0 h-[120px] lg:h-[160px] bg-gradient-to-b from-[#9C1E1E] via-[#9C1E1E]/80 to-transparent z-30 pointer-events-none" />
      ...
      >
        ...
        >
          <div className="relative bg-black shadow-2xl rounded-lg overflow-hidden">
            <div className="aspect-[9/16]">
              ...
              {hasError && !loading && <VideoRetryOverlay onRetry={manualRetry} />}
              {isRecovering && !hasError && !loading && <RecoveringIndicator />}
            </div>
              ...
              <div className="absolute bottom-[12%] lg:bottom-[10%] left-1/2 -translate-x-1/2 flex items-center gap-3 z-[50]">
                <div className="relative">
                  {showSoundTooltip && (
                    <div className="absolute -top-14 left-0 animate-fade-in">
                      <div className="bg-white/95 backdrop-blur-sm px-3 py-2 rounded-lg shadow-lg text-xs font-medium text-exa-black whitespace-nowrap">
                        Ative o som
                        <div className="absolute -bottom-1 left-4 w-2 h-2 bg-white/95 rotate-45" />
                      </div>
                  ...
                  <button
                    onClick={toggleMute}
                    aria-label={isMuted ? 'Ativar som' : 'Desativar som'}
                    className="w-6 h-6 lg:w-7 lg:h-7 rounded-full bg-white/10 backdrop-blur-md border border-white/20 hover:bg-white/20 transition-all duration-300 flex items-center justify-center group"
                  >
                    {isMuted ? (
        ...
        <div className="space-y-5 md:space-y-6 text-center lg:text-left">
          ...
          <p className="font-poppins text-base md:text-lg lg:text-xl text-gray-200 leading-relaxed max-w-xl mx-auto lg:mx-0">
            A EXA conecta marcas aos instantes reais da vida urbana — atenção genuína, presença
            diária e resultados duradouros.
          </p>
          <div className="flex gap-4 pt-4 justify-center lg:justify-start">
            <ExaCTA
              variant="primary"
              size="lg"
              href="https://wa.me/5545991415920?text=Ol%C3%A1%2C%20quero%20mais%20informa%C3%A7%C3%B5es%20sobre%20investir%20em%20pain%C3%A9is."
            >
              Quero Saber Mais
            </ExaCTA>
          </div>
      ...
      {/* Bottom gradient */}
      <div className="absolute bottom-0 left-0 right-0 h-[100px] lg:h-[140px] bg-gradient-to-t from-[#180A0A] via-[#180A0A]/70 to-transparent z-20 pointer-events-none" />
    </section>
  );
};
