        ...
        {loading ? (
          ...
          <>
            ...
            <div className="flex items-center gap-1.5 sm:gap-2 mb-2 sm:mb-3">
              ...
              <span className="text-white/70 text-[10px] sm:text-xs md:text-sm font-medium uppercase tracking-wide truncate">
                FOZ DO IGUAÇU, PR
              </span>
            </div>
            ...
            <div className="flex items-center gap-2 sm:gap-3 md:gap-4 mb-2 sm:mb-3">
              <WeatherIcon 
                ...
                className="h-10 w-10 sm:h-12 sm:w-12 md:h-16 md:w-16 lg:h-20 lg:w-20 text-yellow-400 flex-shrink-0" 
              />
              <div className="flex-1 min-w-0">
                <div className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-bold text-white leading-none mb-0.5 sm:mb-1">
            ...
            <div className="grid grid-cols-3 gap-1 sm:gap-2 md:gap-3 mb-2 sm:mb-3">
              {weatherData.forecast.slice(0, 3).map((day) => (
                <div 
                  key={day.date} 
                  className="text-center p-1 sm:p-2 md:p-2.5 bg-white/5 rounded-md sm:rounded-lg hover:bg-white/10 transition-colors"
                >
                  <div className="text-white/80 text-[9px] sm:text-[10px] md:text-xs font-medium capitalize truncate mb-0.5 sm:mb-1">
                    {day.dayName}
                  </div>
                  <WeatherIcon 
                    icon={day.icon} 
                    className="h-5 w-5 sm:h-6 sm:w-6 md:h-7 md:w-7 lg:h-8 lg:w-8 text-white/80 mx-auto mb-0.5 sm:mb-1" 
                  />
                  <div className="text-white text-[9px] sm:text-[10px] md:text-xs font-semibold">
                    {day.maxTemp}°<span className="text-white/60">/{day.minTemp}°</span>
                  </div>
                  {day.rainChance !== undefined && day.rainChance > 0 && (
                    <div className="text-blue-400 text-[8px] sm:text-[9px] md:text-[10px] mt-0.5">
                      💧 {day.rainChance}%
                    </div>
                  )}
                </div>
            ...
            {/* Indicador "Ao Vivo" */}
            <div className="flex items-center justify-center gap-1.5 sm:gap-2 text-[9px] sm:text-[10px] md:text-xs pt-1.5 sm:pt-2 border-t border-white/5">
