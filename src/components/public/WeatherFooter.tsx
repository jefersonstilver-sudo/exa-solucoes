...
const WeatherFooter: React.FC<WeatherFooterProps> = ({ buildingName }) => {
  ...
  return (
    <footer className="bg-gradient-to-br from-slate-900/80 to-slate-800/80 backdrop-blur-sm rounded-2xl shadow-2xl border border-white/10 p-3">
            ...
            {loading ? (
              ...
              <>
                <div className="flex-shrink-0">
                  <WeatherIcon icon={weatherData.current.icon} className="h-7 w-7 text-yellow-400" />
                </div>
                <div>
                ...
                <div className="flex items-center gap-2 md:gap-3 ml-auto flex-wrap">
                  {weatherData.forecast.slice(0, 3).map((day) => (
                    <div key={day.date} className="text-center p-2 bg-white/5 rounded-lg min-w-[75px]">
                      <div className="text-white/80 text-xs mb-1 capitalize font-medium">
                        {day.dayName}
                      </div>
                      <WeatherIcon icon={day.icon} className="h-4 w-4 text-white/90 mx-auto mb-1" />
