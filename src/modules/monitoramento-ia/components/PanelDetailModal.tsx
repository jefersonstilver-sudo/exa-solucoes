...
export const PanelDetailModal = ({ device, onClose, onUpdate }: PanelDetailModalProps) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-module-primary rounded-2xl shadow-2xl max-w-6xl w-full max-h-[90vh] flex flex-col border border-module">
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-module bg-module-card">
          ...
          <button
            onClick={onClose}
            className="text-module-secondary hover:text-module-primary transition-colors p-2 hover:bg-module-secondary/30 rounded-lg"
          >
            <X className="w-6 h-6" />
          </button>
        </div>
        ...
        {/* Tabs */}
        <div className="flex border-b border-module bg-module-card px-6 overflow-x-auto">
          {tabs.map((tab) => {
            const Icon = tab.icon;
            return (
              <button
                ...
                className={`flex items-center gap-2 px-6 py-4 border-b-2 transition-all whitespace-nowrap font-medium ${
                  activeTab === tab.id
                    ...
                    : 'border-transparent text-module-secondary hover:text-module-primary hover:bg-module-secondary/30'
                }`}
              >
        ...
        {/* Content */}
        <div className="flex-1 overflow-y-auto p-6 bg-module-secondary/30">
          {activeTab === 'informacoes' && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <CardContent className="space-y-4">
                  ...
                  {device.provider && device.provider !== 'Sem provedor' && (
                    <div>
                      ...
                      <Badge className="bg-module-accent/10 text-module-accent border-module-accent/20 font-semibold">
                        {device.provider}
                      </Badge>
                    </div>
                  )}
                ...
                <CardContent className="space-y-4">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      min={1}
                      max={60}
                      className="bg-module-primary border-module text-module-primary"
                    />
                  </div>
                  
                  <Button className="w-full bg-module-accent hover:bg-module-accent/90 text-white font-semibold">
              ...
              <Card className="lg:col-span-3 bg-module-card border-module">
                <CardHeader className="pb-3">
                  <CardTitle className="text-base flex items-center gap-2 text-module-primary">
                ...
                {loadingAlerts ? (
                  ...
                  <div className="space-y-3">
                    {alerts.map((alert) => (
                      <div
                        key={alert.id}
                        className="p-4 border border-module rounded-lg hover:bg-module-secondary/50 transition-colors bg-module-card"
                      >
          ...
          {activeTab === 'alertas' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Card className="bg-module-card border-module hover:border-module-accent/50 transition-colors cursor-pointer hover:shadow-lg">
        ...
        {/* Footer */}
        <div className="flex justify-end gap-3 p-6 border-t border-module bg-module-card">
          <Button
            variant="outline"
            onClick={onClose}
